...
class PopUpViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        averagePriceLabel.text = "1박 평균 가격은 \(model?.cost ?? 0) 원입니다."
    ...
    @IBAction func safariButtonDidTap(_ sender: UIButton) {
        guard let hotelURL = model?.url else { return }
    ...
    @IBAction func minusDay(_ sender: Any) {
        let averagePrice = model?.cost ?? 0
        if Day == 1{
            //Do nothing
        }
        else{
            Day = Day - 1
            sleepDayLabel.text = "\(Day) 박"
            // 자릿수 표현 삽입
            totalP = Day * averagePrice
            totalPriceLabel.text = String((Day * averagePrice).commaRepresentation) + " ₩"
